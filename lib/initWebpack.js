...
function initWebpackConfig(cwd, opts) {
	...
	let defaultConfig = {
		...
		babel: {
			presets: (babel.presets || ['babel-preset-es2015']).map(m => ~m.indexOf('babel-preset') ? utils.moduleResolvePath(m) : path.join(cwd, 'node_modules', `babel-preset-${m}`)),
			plugins: (babel.plugins || ['babel-plugin-transform-runtime']).map(m => ~m.indexOf('babel-plugin') ? utils.moduleResolvePath(m) : path.join(cwd, 'node_modules', `babel-plugin-${m}`))
		},
		postcss: function () {
	...
	if (opts.debug) {
		...
		userOpt.jsUglify === false || defaultConfig.plugins.push(new UglifyJsPlugin({ compress: { warnings: false } }));
	}
