...
function initWebpackConfig(cwd, opts) {
	...
	let defaultConfig = {
		...
		babel: {
			presets: (babel.presets || ['babel-preset-es2015']).map(m => ~m.indexOf('babel-preset') ? utils.moduleResolvePath(m) : m),
			plugins: (babel.plugins || ['babel-plugin-transform-runtime']).map(m => ~m.indexOf('babel-plugin') ? utils.moduleResolvePath(m) : m)
		},
		postcss: function () {
