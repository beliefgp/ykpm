...
module.exports = {
	run: (cwd, args, argv) => {
		...
		let devClient = [`${moduleResolvePath('webpack-dev-server/client')}?http://${serverConfig.hostname}:${serverConfig.port}`];
		if (serverConfig.hot) {
		...
		let compiler = webpack(cwd, webpackConfig);
		let server = webpackServer(compiler, serverConfig);
